...
class SodaMeteorologicalData():
    def load_config(self):
        with open(os.getenv('CONFIG'), 'r') as conf:
            return yaml.safe_load(conf)

    def get_weather_sensor_path(self, xml_data, config):
    ...
    def find_record(self, measurement_label, config):
            ...
            for station in soda_stations:
                ...
                try:
                    ...
                    if request.status_code == 200:
                        weather_sensors = self.get_weather_sensor_path(request.content.decode('utf-8'), config)

                        if weather_sensors is not None:
                            record, timestamp = self.find_record(weather_sensors, config)
...
if __name__ == "__main__":
    soda = SodaMeteorologicalData()
    data = soda.get_data_from_soda()
    # print(data)
